...
class News_Community_2_Seeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $listCommunity = DB::table('community')->where('id', 2)->get();
        $jsonString = file_get_contents(base_path('resources/dataOld/community_2.json'));
        ...
        foreach ($json as $key => $value) {
            if (DB::table('news')->where('title', $value['title'])->count() ==0) {
                DB::table('news')->insert([
                    ...
                    'thumbnail' => config('constants.news_thumbnail_default.2'),
