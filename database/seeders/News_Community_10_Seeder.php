...
class News_Community_10_Seeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $listCommunity = DB::table('community')->where('id', 5)->get();
        $jsonString = file_get_contents(base_path('resources/dataOld/community_10.json'));
        ...
        foreach ($json as $key => $value) {
            if (DB::table('news')->where('title', $value['title'])->count() == 0) {
                DB::table('news')->insert([
                    ...
                    'thumbnail' => config('constants.news_thumbnail_default.10'),
