                    ...
                    <div class="single-content2">
                        <div class="entry-header entry-header-style-1 mb-50">
                            <h1 class="entry-title mb-30 font-weight-900">
                                <section>{!! $vocation->title !!}</section>
                            </h1>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="entry-meta align-items-center meta-2 font-small color-muted">
                                        <p class="mb-5">
                                             <a href="{{route('clients.community.show', [$vocation->commnuity_id ?? "1"])}}"><span class="author-name font-weight-bold">{{$vocation->community->name ?? "Khác"}}</span></a>
                                        </p>
                                        <span class="mr-10">{{convertTimeDbToTimeStringDate($vocation->created_at)}}</span>
                                    </div>
                                </div>
                        ...
                        <figure class="image mb-30 m-auto text-center border-radius-10">
                            <img class="border-radius-10" src="{{getDomainShowImage().$vocation->thumbnail}}" alt="post-title">
                        </figure>
                    </div>
                </div>
...
{{--                            <img class="about-author-img mb-25" src="assets/imgs/authors/author-1.jpg" alt="">--}}
                            <h5 class="mb-20">{!!$vocation->title!!}</h5>
                            <p class="font-medium text-muted">{{$vocation->description}}</p>
                        </div>
                        <div class="sidebar-widget widget-latest-posts mb-50 wow fadeInUp animated">
                            ...
                            <div class="post-block-list post-module-1">
                                <ul class="list-post">
                                    @foreach($listVocationHot as $vocation)
                                    <li class="mb-30 wow fadeInUp animated">
                                        <div class="d-flex bg-white has-border p-25 hover-up transition-normal border-radius-5">
                                            <div class="post-content media-body">
                                                <h6 class="post-title mb-15 text-limit-3-row font-medium"><a
                                                            href="{{ route('clients.vocation.show', [$vocation->id]) }}">
                                                        {!! $vocation->title !!}
                                                    </a></h6>
                                                <div class="entry-meta meta-1 float-left font-x-small text-uppercase">
                                                    <span class="post-on">{{convertTimeDbToTimeStringDate($vocation->date_time)}}</span>
{{--                                                    <span class="post-by has-dot">150 views</span>--}}
                                                </div>
                                            </div>
                                            <div class="post-thumb post-thumb-80 d-flex ml-15 border-radius-5 img-hover-scale overflow-hidden">
                                                <a class="color-white" href="single.html.htm">
                                                    <img src="{{getDomainShowImage().$vocation->thumbnail}}" alt="">
                                                </a>
                                            </div>
                                        </div>
                                    </li>
