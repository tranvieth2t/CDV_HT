<footer id="footer">
    <div class="container">
        <h3 class="text-center">Cộng đoàn Giáo phận Vinh tại Hà Nội</h3>
        <p class="text-center">Được xây dựng bởi nhóm phát triển Diễn đàn và Website Cộng đoàn Giáo phận Vinh tại Hà Nội <br>
            Email chính: congdoanvinhtaihanoi@gmail.com <br>
            Email hỗ trợ: hotro@congdoanvinh.com - hotro@forum.congdoanvinh.com<br></p>
        <div class="social-links d-flex justify-content-center">
            <a href="#" class="facebook"><i class="fa-brands fa-facebook"></i></a>
            <a href="#" class="youtube"><i class="fa-brands fa-youtube"></i></a>
            <a href="#" class="linkedin"><i class="fa-brands fa-facebook"></i></i></a>
        </div>

    </div>
</footer><!-- End Footer -->